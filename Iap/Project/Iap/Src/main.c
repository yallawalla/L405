...
void				SendAck(int a) {
	int i;
						RCC_AHB1PeriphClockCmd(RCC_AHB1Periph_CRC, ENABLE);
						CRC_ResetDR();
						i=CRC_CalcBlockCRC((uint32_t *)0x1FFF7A10,3);
						_timeout=INT_MAX;
						tx.StdId=_ID_IAP_ACK;
						tx.DLC=8;
						memcpy(&tx.Data[0],&a,sizeof(int));
						memcpy(&tx.Data[4],&i,sizeof(int));
...
char	s[128];
						...
						if(f_mount(0,&fs)==FR_OK && f_open(&f,"L405.hex",FA_READ)==FR_OK) {
							for(i=0; i<5; ++i) {
								Watchdog();
