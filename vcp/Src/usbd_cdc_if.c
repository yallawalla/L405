...
static int8_t CDC_Control_FS(uint8_t cmd, uint8_t* pbuf, uint16_t length)
{
  ...
  /*******************************************************************************/
static uint8_t lineCoding[] // 115200bps, 1stop, no parity, 8bit
...
* Return				:
*******************************************************************************/
static void	*CDC_Poll_FS(void *v) {
	_io	*io=*(_io **)v;
  USBD_CDC_HandleTypeDef *hcdc = (USBD_CDC_HandleTypeDef*)hVcpDeviceFS.pClassData;
	if(io) {
		if(hcdc->TxState ==  0) {
		...
		return NULL;
	if(hVcpDeviceFS.dev_state==USBD_STATE_SUSPENDED && hVcpDeviceFS.dev_old_state==USBD_STATE_CONFIGURED)
		*(_io **)v=NULL;
	return v;
}
