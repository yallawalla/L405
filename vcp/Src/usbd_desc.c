...
uint8_t * USBD_VCP_USR_BOSDescriptor(USBD_SpeedTypeDef speed, uint16_t *length);
...
/** BOS descriptor. */
  ...
  #pragma data_alignment=4
#endif /* defined ( __ICCARM__ ) */
__ALIGN_BEGIN uint8_t USBD_VCP_BOSDesc[USB_SIZ_BOS_DESC] __ALIGN_END =
{
  0x5,
  USB_DESC_TYPE_BOS,
  0xC,
...
__ALIGN_BEGIN uint8_t USBD_VCP_StrDesc[USBD_MAX_STR_DESC_SIZ] __ALIGN_END;
  ...
  #pragma data_alignment=4   
#endif
__ALIGN_BEGIN uint8_t USBD_VCP_StringSerial[USB_SIZ_STRING_SERIAL] __ALIGN_END = {
...
uint8_t * USBD_VCP_SerialStrDescriptor(USBD_SpeedTypeDef speed, uint16_t *length)
{
  ...
  *length = USB_SIZ_STRING_SERIAL;
   ...
   * ID */
  Get_SerialNum();
  /* USER CODE BEGIN USBD_VCP_SerialStrDescriptor */
  
  /* USER CODE END USBD_VCP_SerialStrDescriptor */
...
{
  UNUSED(speed);
  *length = sizeof(USBD_VCP_BOSDesc);
  return (uint8_t*)USBD_VCP_BOSDesc;
}
#endif /* (USBD_LPM_ENABLED == 1) */
