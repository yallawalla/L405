...
void HAL_TIM_Base_MspInit(TIM_HandleTypeDef* htim_base)
{
  ...
  /* USER CODE BEGIN TIM12_MspInit 1 */
		__HAL_TIM_ENABLE_IT(htim_base,TIM_IT_UPDATE);
		__HAL_TIM_ENABLE_IT(htim_base,TIM_IT_CC1);
...
void HAL_TIM_PWM_MspInit(TIM_HandleTypeDef* htim_pwm)
{
  ...
  /* USER CODE END TIM2_MspInit 0 */
    ...
    hdma_tim2_up_ch4.Init.MemBurst = DMA_MBURST_SINGLE;
    ...
    /* Several peripheral DMA handle pointers point to the same DMA handle.
     Be aware that there is only one stream to perform all the requested DMAs. */
    __HAL_LINKDMA(htim_pwm,hdma[TIM_DMA_ID_UPDATE],hdma_tim2_up_ch4);
    __HAL_LINKDMA(htim_pwm,hdma[TIM_DMA_ID_CC4],hdma_tim2_up_ch4);
...
/* USER CODE BEGIN 1 */
// imported for /copied from another MX project, tag #8d63d#
